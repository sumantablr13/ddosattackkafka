...
import com.rxcorp.bdf.datalake.dk9.ndh.loadAsset.model.daqe.physical.spark.DaqeSSTTbl1
import com.rxcorp.bdf.datalake.dk9.ndh.loadAsset.model.ods.physical.spark.{NdhFactOdsErrLog, NdhTransSumAcq}
import com.rxcorp.bdf.datalake.dk9.ndh.loadAsset.processor.ods.SupplierRecordsFile
import com.rxcorp.bdf.datalake.dk9.ndh.loadAsset.processor.ods.validations.{EmailAlerts, ValidationRunner}
...
/**
  * @author rdas2 on 9/29/2020
  *
  * */
class SuiteProcessDcpStream_SST1 extends NdhUnitTest{
  // test starts
  ...
  val uuid2 = s"${java.util.UUID.randomUUID()}".split('-').map(_.trim).mkString
  val uuid3 = s"${java.util.UUID.randomUUID()}".split('-').map(_.trim).mkString
  var newEvent : String =""
  ...
  override def beforeAll() {
    ...
    //SST dump table
    val sstDaqe = new DaqeSSTTbl1(appContext)
    sparkExecutor.sql.dropTable(sstDaqe)
    sparkExecutor.sql.createTable(sstDaqe)

    val sstDaqeInfo= this.getClass.getResourceAsStream("/data/daqe/SST/SSTTbl1.csv")
    ...
    if (sstDaqeDf.count() == 0)
      throw new Exception("ERROR: Loaded 0 records from /data/daqe/SST/SSTTbl1.csv")
    sparkExecutor.sql.insertOverwriteTable(sstDaqe, sstDaqeDf, "Mocked existing source data for DAQE dk9_sst_tbl_1_load table", "SB", "", new TimestampConverter())
    ...
    newEvent =
      s"""
         ...
         |  "payload": {
         |    "batchTable": "devl_dk9_ods.dk9_sst_tbl_1_load",
         |    "client": "Denmark",
         |    "kafkaPiggyBack": "{}",
         |    "layoutName": "SST_SO_Load",
         |    "runId": "$uuid1",
         |    "tabToValidate": "dk9_sst_tbl_1_load",
    ...
    val newEvent1 =
      s"""
         ...
         |  "eventType": "QC0 Validations",
         |  "payload": {
         |    "batchTable": "devl_dk9_ods.dk9_sst_tbl_3_load",
         |    "client": "Denmark",
         |    "kafkaPiggyBack": "{}",
         |    "layoutName": "SST_SO_Load",
         |    "runId": "$uuid2",
         |    "tabToValidate": "dk9_sst_tbl_3_load",
         |    "targetSchema": "devl_dk9_ods"
         |  },
         |  "status": "Success",
         |  "ts": 1599753351357
    ...
    val newEvent2 =
      s"""
         ...
         |  "eventType": "QC0 Validations",
         |  "payload": {
         |    "batchTable": "devl_dk9_ods.dk9_sst_tbl_4_load",
         |    "client": "Denmark",
         |    "kafkaPiggyBack": "{}",
         |    "layoutName": "SST_SO_Load",
         |    "runId": "$uuid3",
         |    "tabToValidate": "dk9_sst_tbl_4_load",
         |    "targetSchema": "devl_dk9_ods"
         |  },
         |  "status": "Failure",
         |  "ts": 1599753351357
    ...
    sparkExecutor.kafka.sendEventToKafka(newEvent1, "2nd test kafka message", topicName)
    sparkExecutor.kafka.sendEventToKafka(newEvent2, "3rd test kafka message", topicName)
  ...
  it should "await and process events fine" in {
    mockedService.main(
      Array(
        "-o", "0",
    ...
    assert(spark.table(transSumAcq.tableName).count() == 0)
